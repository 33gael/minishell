...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/25 20:28:14 by thbosvie          #+#    #+#             */
/*   Updated: 2026/04/12 00:09:49 by thbosvie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

t_token	*check_size_in(int *i, t_token *first, t_token *token)
{
	int	size;
...
t_token	*token_loop_in(t_token *token, int *i, int *start, t_token **link)
{
	t_token	*tmp;
	t_token	*first;
	char	*str;
	...
	token = check_size_in(i, first, token);
	if (!token)
		return (NULL);
